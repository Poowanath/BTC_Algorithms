...
class AlgorithmsWithLSTM:
    ...
    def __init__(self,
        ...
        # =========================
        # Split data - Test set = 3 เดือนล่าสุด
        # =========================
        total_len = len(data)
        test_days = test_months * 30  # ประมาณ 90 วัน
        test_start_idx = total_len - test_days
        
        # แบ่ง train/val จากข้อมูลที่เหลือ
        remaining_data = data.iloc[:test_start_idx]
        train_end = int(len(remaining_data) * (train_ratio / (train_ratio + val_ratio)))
        
        self.train_data = remaining_data.iloc[:train_end].copy()
        self.val_data = remaining_data.iloc[train_end:].copy()
        self.test_data = data.iloc[test_start_idx:].copy()

        print("📊 Data Split:")
        print(f"Train: {len(self.train_data)} วัน")
        print(f"Val:   {len(self.val_data)} วัน")
        print(f"Test:  {len(self.test_data)} วัน ({test_months} เดือน)")
    ...
    def _plot_results(self, portfolio, trades, strategy_name, metrics, strategy=None, signals=None):
        ...
        filename = f'picture/with_lstm_short{strategy_name.replace(" ", "_")}.png'
...
if __name__ == "__main__":
    ...
    # สร้าง AlgorithmsWithLSTM โดยใช้ test_months=3 (3 เดือน)
